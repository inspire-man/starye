...
- [x] **Phase 5: 部署基础盘 + 可观测骨架 + Migration 安全** — deploy/rollback workflow、D1 迁移前备份、Sentry 双端接入、RUNBOOK (completed 2026-07-11)
...
**Plans**: 4 plans across 3 waves
  ...
  - [x] 04-01-PLAN.md — Wave 1：统一 `progress` 表 + API/schema/test 基线（PROG-01/02/03）
  - [x] 04-02-PLAN.md — Wave 2：movie 端恢复/保存/完成/历史消费收口（PROG-04/05）
  - [x] 04-03-PLAN.md — Wave 2：comic 端恢复/保存/完成/阅读历史收口（PROG-06/07/08）
  - [x] 04-04-PLAN.md — Wave 3：进度门控统一 + migration/human UAT 回归（PROG-04..08 phase gate）
...
**Plans**: 4 plans across 3 waves
  ...
  - [x] 05-01-PLAN.md — Wave 1：wrangler baseline + rollback workflow + deploy surface 统一（DEPLOY-01/02/06）
  - [x] 05-02-PLAN.md — Wave 2：D1 remote backup + destructive migration gate + 恢复文档（DEPLOY-03/04）
  - [x] 05-03-PLAN.md — Wave 2：Workers Sentry（api/gateway）+ video failure event contract（OBS-01/03/04）
  - [x] 05-04-PLAN.md — Wave 3：frontend Sentry + RUNBOOK 收口 + Phase 5 human ops checklist（OBS-02/05 + DEPLOY-05）
...
| 4. 统一 Progress 表 + 漫画阅读/视频观看进度 | 4/4 | Complete | 2026-05-13 |
| 5. 部署基础盘 + 可观测骨架 + Migration 安全 | 4/4 | Complete    | 2026-05-13 |
