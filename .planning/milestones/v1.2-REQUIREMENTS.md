# Requirements Archive: v1.2 Cloudflare account and domain switching with deferred Viewer proof

**Archived:** 2026-07-29
**Status:** OVERRIDE CLOSEOUT

The milestone archive is complete, but its final audit is `gaps_found` rather
than a production-complete release. The root requirement file is removed at
closeout; the next milestone starts with fresh requirements.

## Final Outcomes

- **Verified:** 26/30 requirements, including target profiles, configuration
  switching, deployment contracts, runbook hardening, and the current 30-row
  evidence matrix.
- **Deferred:** DATA-05, DATA-07, and TEST-05 remain partial; DATA-06 remains
  blocked at the selected-production Viewer checkpoint.
- **Authority:** The canonical status source is
  [v1.2-MILESTONE-AUDIT.md](v1.2-MILESTONE-AUDIT.md).
