...
- **After Wave 1:** Run config, crawler, and DB focused suites; verify the deterministic fixed 10-item fixture set, primary tuple preservation, D1 cardinality, evidence redaction, all three lifecycle states, ordered observer append grammar, and no-mutation gates.
...
| 13-02-02 | 02 | 1 | DATA-03, DATA-04, TEST-05 | T-13-07, T-13-08, T-13-09 | The fixture is a closed 10-item non-R18 set with one player each; the primary item remains the sole correlation tuple and the snapshot/audit is read-only. | unit/contract | `pnpm --filter @starye/crawler exec vitest run src/smoke/__tests__/data-chain-fixture.test.ts src/utils/__tests__/api-client.test.ts && pnpm --filter @starye/db exec vitest run scripts/__tests__/target-d1-mutation.test.ts` | Invalid prepared context, wrong cardinality, or fixture/snapshot gate blocks before any pass claim. | add in task | planned |
...
- [x] Prepared-entry/crawler/DB tests cover the closed 10-item one-player-per-item fixture scope, primary tuple, read-only cardinality audit, no mutation after failed gate, and secret-free child output.
