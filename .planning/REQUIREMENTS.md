...
- [x] **PROD-02**: Actions 将应用 run 与 `GITHUB_RUN_ID` 绑定，并以签名事件回写状态、日志和终态 receipt。
- [x] **PROD-03**: 生产任务支持 provider 状态补偿、取消和重试，且不会把 dispatch 受理视为成功。

### 后台与内容运维

- [x] **DASH-01**: Dashboard 支持创建、列表、详情、分页日志和状态自动刷新。
- [x] **DASH-02**: Dashboard 支持确认后的取消/重试，并继承现有视频/漫画资源权限。
- [x] **DASH-03**: 成功 receipt 链接到现有视频/漫画内容 CRUD，管理员可完成增删改。
