...
- 默认使用中文进行协作沟通、问题分析和结果反馈。
- 对外回复优先使用中文说明结论、假设、风险、验证结果和下一步动作。
- 实现说明应以中文记录关键决策和变更原因，方便后续维护与交接。
- 不要求记录或暴露内部逐步推理过程；文档中只保留可执行结论、实现摘要和验证信息。
...
This project is indexed by GitNexus as **starye** (17230 symbols, 22604 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
