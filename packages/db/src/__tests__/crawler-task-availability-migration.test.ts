...
describe('crawler availability migration', () => {
  ...
  it('retains observation history while current has one target row and duplicate identities are rejected', async () => {
    ...
    const readback = await client.execute('SELECT observation_identity, source_revision, policy_version FROM crawler_availability_current WHERE target_kind = \'movie\' AND target_id = \'movie-1\' LIMIT 1')
    expect(readback.rows).toEqual([{ observation_identity: 'identity-1', source_revision: 3, policy_version: 'v1' }])
    const history = await client.execute('SELECT count(*) AS count FROM crawler_availability_observation WHERE task_id = \'task-1\' LIMIT 50')
