...
async function main() {
  // 从环境变量读取配置
  const config: ActorCrawlerConfig = {
    browserConfig: {
      puppeteer: {
    ...
    maxActors: Number.parseInt(process.env.MAX_ACTORS || '150'),
    concurrency: Number.parseInt(process.env.ACTOR_CONCURRENCY || '2'),
    delay: Number.parseInt(process.env.ACTOR_DELAY || '8000'),
    recoveryMode: process.env.RECOVERY_MODE === 'true',
  }

  console.log('🎬 启动女优详情爬虫')
  console.log('📊 配置信息:')
  console.log(`  最大女优数: ${config.maxActors}`)
  ...
  // 创建并运行爬虫
  const crawler = new ActorCrawler(config)
