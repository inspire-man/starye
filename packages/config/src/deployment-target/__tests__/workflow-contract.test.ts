...
describe('phase 18 GitHub Actions crawler orchestration contract', () => {
  it('keeps movie and manga schedule/manual entries inside the signed control plane', async () => {
    for (const file of ['daily-movie-crawl.yml', 'daily-manga-crawl.yml']) {
      ...
      expect(source, file).toContain('workflow_dispatch:')
      expect(source, file).toContain('registration_probe:')
      expect(source, file).toContain('inputs.registration_probe == true')
      expect(source, file).toContain('INPUT_REGISTRATION_PROBE:')
      expect(source, file).toContain('needs.register-schedule.result == \'success\'')
      expect(source, file).toContain('scheduled_at="$(date -u +%Y-%m-%dT%H:%M:%S.000Z)"')
      expect(source, file).toContain('schedule_bucket="$' + '{scheduled_at:0:13}:00Z"')
      expect(source, file).not.toContain('scheduled_at="$(date -u +%Y-%m-%dT%H:00:00.000Z)"')
      ...
      expect(source, file).not.toContain('actions-event-client.ts provider-started')
      expect(source, file).not.toContain('actions-event-client.ts progress 2')
