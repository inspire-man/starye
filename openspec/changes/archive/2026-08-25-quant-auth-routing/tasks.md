...
- [x] 3.2 通过 Gateway、浏览器、聚焦测试、类型检查和构建验证；完成标准：未登录不进入 `/blog/quant/`，已登录 Quant 保持可用。当前 Gateway/Auth/本地启动器测试、Quant 类型检查和构建均通过；Auth Nuxt typecheck 仍受既有 `packages/config` `.ts` 扩展配置错误影响。
