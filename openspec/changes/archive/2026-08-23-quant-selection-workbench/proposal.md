...
- 在观察池返回最新收盘价、涨跌幅，并在前端增加候选筛选、信号解释、数据覆盖和风险提示。
- 将积分、能力注册表、provider 和管理员会话从择股工作台移出；这些信息归数据源/运管能力页面负责。
...
- Frontend：Quant App 的布局、观察池、候选快照、数据状态和风险展示；页面不再加载运管能力接口。
...
- 本轮不实现回测、组合收益归因和自动交易。
- 本轮不删除能力接口；能力接口仍作为数据源/运管侧的诊断契约保留。
