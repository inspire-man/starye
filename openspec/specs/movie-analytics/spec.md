# Spec：影片内容洞察（管理端 Analytics）

## 目标

为管理端首页提供内容运营洞察，展示热门影片排行和 Genre 分布，帮助管理员了解内容热度与类型构成。

## 接口规格

### GET /api/admin/movies/analytics

#### 权限
仅限管理员（`admin` 或 `movie_admin` 角色），非管理员 MUST 返回 403。
...
- **THEN** MUST 返回 `{ hotMovies: [...], ... }`，hotMovies 最多 10 条，按 viewCount DESC 排序
