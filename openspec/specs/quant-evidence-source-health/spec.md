...
Quant 因子数据健康 MUST 以 evidence 的有限原始 `value` 和来源状态判定字段是否已读取；`pass`、`caution` 和带有限值的 `fail` 都属于已读取字段。阈值未通过 MUST 保留为失败证据和研究风险，但 MUST NOT 单独把字段标记为缺失、来源不可用或要求重试。
...
Quant provider 收到合法的空报告响应时 MUST 返回空历史或空集合并保留其他字段；只有响应结构非法、请求超时或上游错误才返回 provider error code。空历史 MUST 在股东回报区域显示为数据不足，不得显示为来源不可用。
