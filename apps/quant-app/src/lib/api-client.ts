...
function parseAiFactorReviews(value: unknown): QuantAiFactorReview[] | null {
  if (value === undefined)
    return []
...
export const quantApi = {
  ...
  async getResearchDecisionQueue(limit = 20): Promise<QuantDecisionRecord[]> {
    return parseDecisionRecords(await requestJson(`/research/decisions?limit=${encodeURIComponent(String(limit))}`))
  },
