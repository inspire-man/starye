...
export const ChapterDetailSchema = v.pipe(
  v.object({
    ...
    pageAvailability: v.optional(v.nullable(v.object({
      availablePageCount: v.number(),
      expectedPageCount: v.number(),
      projectionVersion: v.number(),
      sourceRevision: v.number(),
      status: v.picklist(['available', 'unavailable', 'degraded', 'unknown']),
      storedPageCount: v.number(),
      unknownPageCount: v.number(),
      unavailablePageCount: v.number(),
      policyVersion: v.string(),
      observedAt: v.union([v.string(), v.number()]),
