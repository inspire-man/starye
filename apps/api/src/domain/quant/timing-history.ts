...
export function buildTimingHistory(bars: readonly DailyBar[]): TimingHistory {
  ...
  for (let anchorIndex = HISTORY_DAYS - 1; anchorIndex + FORWARD_DAYS < chronologicalBars.length; anchorIndex += FORWARD_DAYS) {
    ...
    if (!anchor || !future || anchor.close <= 0)
      continue
...
export const TIMING_HISTORY_FORMULA_VERSION = 'timing-history-v1' as const

export function timingHistoryEdgeStatus(edge: TimingHistoryEdgeAssessment): 'pass' | 'caution' | 'fail' | 'missing' {
  if (edge === 'supported')
    return 'pass'
  if (edge === 'weaker')
    return 'fail'
  if (edge === 'indeterminate')
    return 'caution'
  return 'missing'
}

export function timingHistoryEdgeDetail(edge: TimingHistoryEdgeAssessment): string {
  if (edge === 'supported')
    return '相对基准有稳定支持'
  if (edge === 'weaker')
    return '相对基准偏弱'
...
export function currentTimingHistoryBucket(history: TimingHistory): TimingHistoryBucket | null {
