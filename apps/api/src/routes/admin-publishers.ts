...
adminPublishers.get('/stats', async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      withSourceUrl: withSourceUrlCount, // 有 sourceUrl 的厂商（可以爬取详情）
...
adminPublishers.get(
  ...
  zValidator('query', z.object({
    ...
    onlyPending: z.enum(['true', 'false']).optional(), // 仅显示待爬取详情的厂商
  })),
  async (c) => {
    ...
    try {
      ...
      if (onlyPending === 'true') {
        // 仅显示 hasDetailsCrawled = false 的厂商
        whereClause = search
          ? and(eq(publishers.hasDetailsCrawled, false), like(publishers.name, `%${search}%`))
          : eq(publishers.hasDetailsCrawled, false)
      }
      else {
        whereClause = search ? like(publishers.name, `%${search}%`) : undefined
      }
