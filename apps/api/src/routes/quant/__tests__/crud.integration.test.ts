...
describe('quant watchlist CRUD contract', () => {
  ...
  it('records an owned decision with a server-built snapshot and isolated history', async () => {
    ...
    const calibrationA = await userA.request('/api/quant/research/decisions/calibration?limit=100')
    expect(calibrationA.status).toBe(200)
    await expect(calibrationA.json()).resolves.toMatchObject({ data: expect.arrayContaining([expect.objectContaining({ action: 'holding' })]) })
    expect((await userB.request('/api/quant/research/decisions/calibration?limit=100')).status).toBe(200)
