...
if (!pagesBuildEnvPath) {
  throw new Error('STARYE_PAGES_BUILD_ENV_PATH is required for the comic public runtime build.')
}

const publicRuntimeEnv = buildVitePublicRuntimeEnv(
  parseVitePublicRuntimeEnv(parsePagesBuildEnvText(readFileSync(pagesBuildEnvPath, 'utf8'), 'comic'), 'comic'),
  'comic',
)
...
export default defineConfig(() => ({
  ...
  define: {
    __STARYE_COMIC_PUBLIC_RUNTIME__: JSON.stringify({
