...
const actorSlug = computed(() => route.params.name as string)
...
const { data: response, pending, error } = useApi<ActorData>(`/movies/actors/${actorSlug.value}`, {
  query: {
    page,
    limit: 24,
  },
...
const actorData = computed(() => response.value?.data)
const movies = computed(() => actorData.value?.movies || [])
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950">
    <div class="container mx-auto px-4 py-12">
      ...
      <!-- Content -->
      <div v-else-if="actorData" class="space-y-8">
        <!-- Actor Header -->
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-purple-500/20 to-pink-500/20 border border-purple-500/30 p-8 md:p-12">
          <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-purple-500/20 via-transparent to-transparent" />

          <div class="relative flex flex-col md:flex-row items-center md:items-start gap-6">
            <!-- Avatar -->
            <div class="w-32 h-32 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center text-white text-5xl font-bold shadow-2xl">
              {{ actorData.name.charAt(0) }}
            </div>
            ...
            <div class="flex-1 text-center md:text-left">
              ...
              <div class="flex flex-wrap items-center justify-center md:justify-start gap-4 text-slate-300">
                <div class="flex items-center gap-2">
                  ...
                  <span class="font-bold">{{ actorData.movieCount }} {{ t('movie.works_count') }}</span>
                </div>
              </div>
            </div>
          </div>
