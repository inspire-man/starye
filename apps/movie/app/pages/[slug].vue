...
<template>
  <div class="container mx-auto py-12 px-4 max-w-6xl">
    ...
    <div v-else-if="error || !movie" class="p-12 text-center border border-dashed rounded-3xl">
       <h2 class="text-2xl font-bold text-muted-foreground">{{ $t('common.error') }}</h2>
       <p class="mt-2 text-sm text-muted-foreground opacity-70">{{ error?.message || 'Movie not found' }}</p>
       <NuxtLink to="/" class="mt-6 inline-block text-primary font-bold hover:underline">Back to Library</NuxtLink>
    </div>
      ...
      <div class="lg:col-span-8 space-y-8">
        <div class="relative aspect-video rounded-3xl overflow-hidden shadow-2xl border border-border/40 bg-black flex items-center justify-center">
          ...
          <div v-else class="flex flex-col items-center justify-center text-center p-8">
             <div class="w-16 h-16 bg-destructive/10 text-destructive rounded-full flex items-center justify-center text-2xl mb-4">🔞</div>
             <p class="font-bold text-xl mb-2">{{ $t('movie.adult_only') }}</p>
             <p class="text-sm text-muted-foreground max-w-xs mx-auto">
               Age verification required. Please login to view sensitive content.
             </p>
             <NuxtLink to="/login" class="mt-6 px-6 py-2 bg-primary text-primary-foreground rounded-full text-sm font-bold">Login to Verify</NuxtLink>
          </div>
        </div>
      ...
      <div class="lg:col-span-4 space-y-8">
        <div class="p-8 bg-muted/20 border border-border/40 rounded-3xl space-y-8">
          <div v-if="movie.publisher" class="space-y-1.5">
            <p class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{{ $t('movie.publisher') }}</p>
            <p class="font-extrabold text-sm">{{ movie.publisher }}</p>
          </div>

          <div v-if="movie.releaseDate" class="space-y-1.5">
            <p class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{{ $t('movie.release_date') }}</p>
            <p class="font-extrabold text-sm">{{ formatDate(movie.releaseDate) }}</p>
          </div>

          <div v-if="movie.duration" class="space-y-1.5">
            <p class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{{ $t('movie.duration') }}</p>
            <p class="font-extrabold text-sm">{{ movie.duration }} {{ $t('movie.minutes') }}</p>
          </div>

          <div v-if="movie.series" class="space-y-1.5">
            <p class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{{ $t('movie.series') }}</p>
            <p class="font-extrabold text-sm">{{ movie.series }}</p>
          </div>

          <div v-if="movie.actors && movie.actors.length" class="space-y-3">
            <p class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{{ $t('movie.actors') }}</p>
            <div class="flex flex-wrap gap-2">
