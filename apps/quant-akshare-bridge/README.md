...
- Business segment context maps `stock_zygc_em` product, industry, and region rows to same-period `business_segments` with raw revenue, revenue ratio, cost, cost ratio, profit, profit ratio, and source-reported gross margin; non-finite fields remain `null` and do not infer orders, volume, or realized price.
