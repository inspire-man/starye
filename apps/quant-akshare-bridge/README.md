...
- The response is `quant-akshare-v1`; missing upstream fields stay `null` and provider failures are classified in `errors`. Newer bridge builds may add `cashflows`, `repurchases`, and `dividends` without breaking older Worker clients.
