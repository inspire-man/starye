...
- The request accepts `ts_code`, optional `start_date`/`end_date`, `include_financials`, and `include_capital_structures`.
- The response is `quant-akshare-v1`; missing upstream fields stay `null` and provider failures are classified in `errors`. Newer bridge builds may add `cashflows`, `capital_structures`, `repurchases`, and `dividends` without breaking older Worker clients.
