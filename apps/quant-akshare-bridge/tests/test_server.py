...
class ServerTest(unittest.TestCase):
    ...
    def test_authenticated_evidence_returns_normalized_json(self) -> None:
        ...
        collect.assert_called_once_with(BridgeRequest(ts_code="601899.SH", start_date=None, end_date=None, include_financials=True, include_capital_structures=True, include_profit_forecasts=True))
    ...
    def test_rejects_non_boolean_profit_forecast_switch(self) -> None:
        status, payload = self.request("POST", "/v1/evidence", {"ts_code": "601899.SH", "include_profit_forecasts": "false"})
        self.assertEqual(status, 400)
        self.assertEqual(payload["errors"][0]["code"], "BRIDGE_INVALID_INPUT")
