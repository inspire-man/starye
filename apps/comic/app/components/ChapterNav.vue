  ...
  <div>
    ...
    >
      <div class="flex justify-between items-center mb-4 sticky top-0 bg-neutral-900 z-10 py-2 border-b border-white/5">
        ...
        <button class="p-2 hover:bg-white/10 rounded-full" @click="isOpen = false">
    ...
    <!-- Toolbar -->
    <div class="fixed bottom-0 inset-x-0 z-50 bg-black/80 backdrop-blur-xl border-t border-white/10 px-4 py-3 flex items-center justify-between gap-4 safe-area-bottom">
      <NuxtLink
        v-if="prev"
        :to="`/${slug}/${prev.slug}`"
        replace
      ...
      <button
        class="px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl flex flex-col items-center justify-center min-w-[60px]"
        @click="isOpen = !isOpen"
      >
