...
> Status: historical. Replaced by the API routes and the protected /api/docs and /api/openapi.json endpoints.
...
- **Base URL**: Gateway origin + `/api`（本地验证入口：`http://localhost:8080/api`）
- **数据格式**: JSON
- **认证方式**: Session Cookie
- **字符编码**: UTF-8
